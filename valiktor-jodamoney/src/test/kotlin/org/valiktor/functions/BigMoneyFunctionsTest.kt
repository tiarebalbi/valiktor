...
private fun <T : Number> moneyOf(
    currency: CurrencyUnit,
    number: T,
) = BigMoney.of(currency, number.asBigDecimal())

private val ONE_NUMBERS =
    listOf<Number>(
        1.toByte(),
        1.toShort(),
        1,
...
private val NEGATIVE_ONE_NUMBERS =
    listOf<Number>(
        1.unaryMinus().toByte(),
        1.unaryMinus().toShort(),
        1.unaryMinus(),
        1.unaryMinus().toLong(),
        1.unaryMinus().toBigInteger(),
        1.unaryMinus().toFloat(),
        1.unaryMinus().toDouble(),
        1.unaryMinus().toBigDecimal(),
    )
...
class BigMoneyFunctionsTest {
    ...
    fun `isNotEqualTo with same value and 2 decimal digits should be invalid`() {
        ...
        assertThat(exception.constraintViolations).containsExactly(
            DefaultConstraintViolation(
                ...
                constraint = NotEquals(moneyOf(USD, 1.00.toBigDecimal())),
            ),
        )
    }
    ...
    fun `isIn vararg with different value should be invalid`() {
        ...
        assertThat(exception.constraintViolations).containsExactly(
            DefaultConstraintViolation(
                property = "salary",
                value = moneyOf(USD, ONE),
                constraint = In(setOf(moneyOf(USD, ZERO), moneyOf(USD, TEN))),
            ),
        )
    }
    ...
    fun `isIn vararg with different currency should be invalid`() {
        ...
        assertThat(exception.constraintViolations).containsExactly(
            DefaultConstraintViolation(
                property = "salary",
                value = moneyOf(USD, ONE),
                constraint = In(setOf(moneyOf(BRL, ONE), moneyOf(USD, TEN))),
            ),
        )
    }
    ...
    fun `isIn iterable with different value should be invalid`() {
        ...
        assertThat(exception.constraintViolations).containsExactly(
            DefaultConstraintViolation(
                ...
                constraint = In(listOf(moneyOf(BRL, ZERO), moneyOf(BRL, TEN))),
            ),
        )
    }
    ...
    fun `isIn iterable with different currency should be invalid`() {
        ...
        assertThat(exception.constraintViolations).containsExactly(
            DefaultConstraintViolation(
                property = "salary",
                value = moneyOf(BRL, ONE),
                constraint = In(listOf(moneyOf(USD, ONE), moneyOf(BRL, TEN))),
            ),
        )
    }
    ...
    fun `isNotIn vararg with same value should be invalid`() {
        ...
        assertThat(exception.constraintViolations).containsExactly(
            DefaultConstraintViolation(
                property = "salary",
                value = moneyOf(USD, ONE),
                constraint = NotIn(setOf(moneyOf(USD, ZERO), moneyOf(USD, ONE), moneyOf(USD, TEN))),
            ),
        )
    }
    ...
    fun `isNotIn vararg with same value and 2 decimal digits should be invalid`() {
        ...
        assertThat(exception.constraintViolations).containsExactly(
            DefaultConstraintViolation(
                property = "salary",
                value = moneyOf(USD, ONE),
                constraint = NotIn(setOf(moneyOf(USD, ZERO), moneyOf(USD, 1.00.toBigDecimal()), moneyOf(USD, TEN))),
            ),
        )
    }
    ...
    fun `isNotIn vararg with same value and different type should be invalid`() {
        ...
        assertThat(exception.constraintViolations).containsExactly(
            DefaultConstraintViolation(
                property = "salary",
                value = moneyOf(USD, ONE),
                constraint = NotIn(setOf(moneyOf(USD, 0L), moneyOf(USD, 1L), moneyOf(USD, 10L))),
            ),
        )
    }
    ...
    fun `isNotIn iterable with same value should be invalid`() {
        ...
        assertThat(exception.constraintViolations).containsExactly(
            DefaultConstraintViolation(
                property = "salary",
                value = moneyOf(BRL, ONE),
                constraint = NotIn(listOf(moneyOf(BRL, ZERO), moneyOf(BRL, ONE), moneyOf(BRL, TEN))),
            ),
        )
    }
    ...
    fun `isNotIn iterable with same value and 2 decimal digits should be invalid`() {
        ...
        assertThat(exception.constraintViolations).containsExactly(
            DefaultConstraintViolation(
                property = "salary",
                value = moneyOf(USD, ONE),
                constraint = NotIn(listOf(moneyOf(USD, ZERO), moneyOf(USD, 1.00.toBigDecimal()), moneyOf(USD, TEN))),
            ),
        )
    }
    ...
    fun `isNotIn iterable with same value and different type should be invalid`() {
        ...
        assertThat(exception.constraintViolations).containsExactly(
            DefaultConstraintViolation(
                property = "salary",
                value = moneyOf(USD, ONE),
                constraint = NotIn(listOf(moneyOf(USD, 0.0), moneyOf(USD, 1.0), moneyOf(USD, 10.0))),
            ),
        )
    }
    ...
    fun `isBetween with equal negative start value should be valid`() {
        validate(Employee(salary = moneyOf(BRL, 2.0.unaryMinus().toBigDecimal()))) {
            validate(
                Employee::salary,
            ).isBetween(start = moneyOf(BRL, 2.0.unaryMinus().toBigDecimal()), end = moneyOf(BRL, 1.0.unaryMinus().toBigDecimal()))
        }
    }
    ...
    fun `isBetween with equal negative end value should be valid`() {
        validate(Employee(salary = moneyOf(USD, 1.0.unaryMinus().toBigDecimal()))) {
            validate(
                Employee::salary,
            ).isBetween(start = moneyOf(USD, 2.0.unaryMinus().toBigDecimal()), end = moneyOf(USD, 1.0.unaryMinus().toBigDecimal()))
        }
    }
    ...
    fun `isBetween with less negative start value should be invalid`() {
        val exception =
            assertFailsWith<ConstraintViolationException> {
                validate(Employee(salary = moneyOf(USD, 10.0.unaryMinus().toBigDecimal()))) {
                    validate(
                        Employee::salary,
                    ).isBetween(start = moneyOf(USD, 9.9.unaryMinus().toBigDecimal()), end = moneyOf(USD, 8.0.unaryMinus().toBigDecimal()))
                }
            }
        ...
        assertThat(exception.constraintViolations).containsExactly(
            DefaultConstraintViolation(
                ...
                constraint =
                    Between(
                        start = moneyOf(USD, 9.9.unaryMinus().toBigDecimal()),
                        end = moneyOf(USD, 8.0.unaryMinus().toBigDecimal()),
                    ),
            ),
        )
    }
            ...
            assertFailsWith<ConstraintViolationException> {
                validate(Employee(salary = moneyOf(BRL, 12.0.unaryMinus().toBigDecimal()))) {
                    validate(
                        ...
                        start = moneyOf(BRL, 13.0.unaryMinus().toBigDecimal()),
                        end = moneyOf(BRL, 12.9.unaryMinus().toBigDecimal()),
                    )
                }
            }
    ...
    fun `isNotBetween with less negative start value should be valid`() {
        validate(Employee(salary = moneyOf(BRL, 10.0.unaryMinus().toBigDecimal()))) {
            validate(
                Employee::salary,
            ).isNotBetween(start = moneyOf(BRL, 9.9.unaryMinus().toBigDecimal()), end = moneyOf(BRL, 8.0.unaryMinus().toBigDecimal()))
        }
    }
    ...
    fun `isNotBetween with greater negative end value should be valid`() {
        validate(Employee(salary = moneyOf(USD, 12.0.unaryMinus().toBigDecimal()))) {
            validate(
                Employee::salary,
            ).isNotBetween(start = moneyOf(USD, 13.0.unaryMinus().toBigDecimal()), end = moneyOf(USD, 12.9.unaryMinus().toBigDecimal()))
        }
    }
            ...
            assertFailsWith<ConstraintViolationException> {
                validate(Employee(salary = moneyOf(BRL, 2.0.unaryMinus().toBigDecimal()))) {
                    validate(
                        ...
                        start = moneyOf(BRL, 2.0.unaryMinus().toBigDecimal()),
                        end = moneyOf(BRL, 1.0.unaryMinus().toBigDecimal()),
                    )
                }
            }
            ...
            assertFailsWith<ConstraintViolationException> {
                validate(Employee(salary = moneyOf(USD, 1.0.unaryMinus().toBigDecimal()))) {
                    validate(
                        ...
                        start = moneyOf(USD, 2.0.unaryMinus().toBigDecimal()),
                        end = moneyOf(USD, 1.0.unaryMinus().toBigDecimal()),
                    )
                }
            }
