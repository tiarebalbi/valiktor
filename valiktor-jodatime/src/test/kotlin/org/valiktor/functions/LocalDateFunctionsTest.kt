    ...
    fun `isEqualTo with different value should be invalid`() {
        ...
        assertThat(exception.constraintViolations).containsExactly(
            DefaultConstraintViolation(
                ...
                value = LocalDate.now(),
                constraint = Equals(LocalDate.now().minusDays(1)),
            ),
        )
    }
    ...
    fun `isIn vararg with different value should be invalid`() {
        ...
        assertThat(exception.constraintViolations).containsExactly(
            DefaultConstraintViolation(
                property = "dateOfBirth",
                value = LocalDate.now(),
                constraint = In(setOf(LocalDate.now().minusDays(1), LocalDate.now().minusDays(2))),
            ),
        )
    }
    ...
    fun `isIn iterable with different value should be invalid`() {
        ...
        assertThat(exception.constraintViolations).containsExactly(
            DefaultConstraintViolation(
                property = "dateOfBirth",
                value = LocalDate.now(),
                constraint = In(listOf(LocalDate.now().minusDays(1), LocalDate.now().minusDays(2))),
            ),
        )
    }
    ...
    fun `isNotIn vararg with same value should be invalid`() {
        ...
        assertThat(exception.constraintViolations).containsExactly(
            DefaultConstraintViolation(
                property = "dateOfBirth",
                value = LocalDate.now(),
                constraint = NotIn(setOf(LocalDate.now(), LocalDate.now().plusDays(1))),
            ),
        )
    }
    ...
    fun `isNotIn iterable with same value should be invalid`() {
        ...
        assertThat(exception.constraintViolations).containsExactly(
            DefaultConstraintViolation(
                property = "dateOfBirth",
                value = LocalDate.now(),
                constraint = NotIn(listOf(LocalDate.now(), LocalDate.now().plusDays(1))),
            ),
        )
    }
