...
class ZonedDateTimeFunctionsTest {
    ...
    fun `isNotToday with 00h00m00 should be invalid`() {
        ...
        assertThat(exception.constraintViolations).containsExactly(
            DefaultConstraintViolation(
                property = "dateOfBirth",
                value = LocalDate.now(ZoneId.systemDefault()).atStartOfDay(ZoneId.systemDefault()),
                constraint = NotToday,
            ),
        )
    }
    ...
    fun `isNotToday with 23h59m59 should be invalid`() {
        ...
        assertThat(exception.constraintViolations).containsExactly(
            DefaultConstraintViolation(
                property = "dateOfBirth",
                value = LocalDate.now(ZoneId.systemDefault()).atTime(23, 59, 59).atZone(ZoneId.systemDefault()),
                constraint = NotToday,
            ),
        )
    }
